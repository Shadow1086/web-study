...
```css
background:
    url("../../资源文件/background/bg-lt.png") no-repeat left top,
...
![](https://cdn.jsdelivr.net/gh/Shadow1086/myPicture@master/uPic/2026/03/29/19-22-fRTACS)
...
1. 开启 BFC 后，其子元素不会产生 margin 塌陷问题。
2. 自己不会倍其他浮动元素所覆盖。
...
- 伸缩项目
- 多列容器
- `column-span` 为 `all` 的元素(即使改元素没有包裹在多列容器中)
- `diplay` 的值设置为 `flow-root`
