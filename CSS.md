...
1. link标签放在head标签中
2. link标签的属性说明：
    - rel属性值为stylesheet，表示引入的文档与当前文档之间的关系
    - href属性值为CSS文件的路径，引入的文档来源于哪里
3. 外部样式的优点：
    - 实现了结构与样式之间的完全分离
    - 多个HTML页面可以复用样式，可以触发浏览器的缓存机制，提高访问速度
...
5. `:focus` 元素获得焦点的状态
    - 表单类元素才能使用 `:focus` 伪类
    - 当用户点击元素，触摸元素，或通过键盘的tab键等方式选择元素时，就是获得焦点
...
#### 16.8.1 padding 复合属性的使用规则(按照顺时针方向)
...
/* 表示标题是否跨列，若为all则跨列，若为none则不跨列(默认) */
column-span: all
